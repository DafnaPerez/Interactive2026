...
function platformLockSplashLogoColors(svg) {
  ...
  Array.from(svg.querySelectorAll("polygon, path")).forEach((el) => {
    ...
    el.style.setProperty("-webkit-forced-color-adjust", "none", "important");
  });
}
...
function platformIsAndroidChrome() {
  ...
  // Samsung Internet only — Google Chrome on Samsung phones should still match.
...
function platformGetSplashViewportWidth() {
  let vv =
...
function platformApplySplashLogoStageSize(stage) {
  ...
  // Use computed px — some Samsung Chrome builds ignore CSS min(vw, px) and
  // the stage collapses to a tiny intrinsic SVG size.
  let screenW = platformGetSplashViewportWidth();
  let w;
  if (platformIsAndroidChrome()) {
    // Larger guaranteed size; matches the non-Chrome Android browser look.
    w = Math.min(Math.round(screenW * 0.72), 300);
  } else {
    // Original intent: min(58vw, 240px)
    w = Math.min(Math.round(screenW * 0.58), 240);
  }
  let h = Math.round(w * (318.5 / 297.81));
...
function platformCommitSplashPieceRestPose(el) {
  if (!el) {
    return;
  }
  el.style.opacity = "1";
...
function platformEnsureSplashOverlay() {
  ...
  if (svg) {
    ...
    platformLockSplashLogoColors(svg);
    // Hide every piece before first paint so we never flash the assembled logo.
    Array.from(svg.querySelectorAll("polygon, path")).forEach((el) => {
      el.style.opacity = "0";
      el.style.transformOrigin = "center";
      // view-box is more reliable than fill-box on Samsung Chrome.
      el.style.transformBox = platformIsAndroidChrome() ? "view-box" : "fill-box";
      ...
      if (anim && anim.finished && typeof anim.finished.then === "function") {
        anim.finished
          .then(() => {
            ...
            platformCommitSplashPieceRestPose(el);
          })
          .catch(() => platformCommitSplashPieceRestPose(el));
      } else {
        setTimeout(() => platformCommitSplashPieceRestPose(el), delay + dur + 30);
      }
    } else {
      platformCommitSplashPieceRestPose(el);
    }
  ...
  paths.forEach((el, i) => {
    ...
    if (typeof el.animate === "function") {
      ...
      if (anim && anim.finished && typeof anim.finished.then === "function") {
        ...
        setTimeout(() => platformCommitSplashPieceRestPose(el), delay + 720 + 30);
      }
    } else {
      platformCommitSplashPieceRestPose(el);
    }
